...
cd /tmp/gh-aw/mcp-scripts || exit 1
...
if [ ! -f mcp-server.cjs ]; then
  echo "ERROR: mcp-server.cjs not found in /tmp/gh-aw/mcp-scripts"
  ls -la /tmp/gh-aw/mcp-scripts/
  exit 1
fi
if [ ! -f tools.json ]; then
  echo "ERROR: tools.json not found in /tmp/gh-aw/mcp-scripts"
  ls -la /tmp/gh-aw/mcp-scripts/
  exit 1
fi
...
if [ ${#MISSING_FILES[@]} -gt 0 ]; then
  echo "ERROR: Missing required dependency files in /tmp/gh-aw/mcp-scripts/"
