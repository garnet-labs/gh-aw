...
case "$ENGINE_TYPE" in
  ...
  gemini)
    ...
    bash /tmp/gh-aw/actions/convert_gateway_config_gemini.sh
    ;;
  *)
...
if [ -f /tmp/gh-aw/actions/check_mcp_servers.sh ]; then
  ...
  set -o pipefail
  if ! bash /tmp/gh-aw/actions/check_mcp_servers.sh \
  ...
  echo "WARNING: MCP server check script not found at /tmp/gh-aw/actions/check_mcp_servers.sh"
