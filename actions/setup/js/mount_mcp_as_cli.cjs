 ...
 */
function generateCLIWrapperScript(serverName, serverUrl, toolsFile) {
  // We use a template literal but avoid single quotes in the embedded bash
  // to keep the heredoc-free approach clean. The API key is read at runtime
  // from the MCP_GATEWAY_API_KEY environment variable.
...
async function main() {
  ...
  if (!apiKey) {
    core.warning("MCP_GATEWAY_API_KEY is not set; generated CLI wrappers will fail at runtime");
  }
  ...
  for (const server of servers) {
    ...
    try {
      fs.writeFileSync(scriptPath, generateCLIWrapperScript(name, url, toolsFile), { mode: 0o755 });
