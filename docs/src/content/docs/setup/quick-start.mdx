...
Here is an example of what the wizard looks like:

```text wrap
✔ Checking repository permissions... done
✔ Select an AI engine › Copilot
✔ Set up COPILOT_GITHUB_TOKEN secret?  › Yes
ℹ To use Copilot as your AI engine you need to add a secret named COPILOT_GITHUB_TOKEN
  with a GitHub Personal Access Token (PAT) that has the Copilot permission enabled.
  See https://github.github.com/gh-aw/reference/auth/#copilot_github_token for details.
✔ Enter your GitHub PAT › ********
✔ Secret COPILOT_GITHUB_TOKEN added successfully
✔ Adding workflow daily-repo-status... done
✔ Trigger an initial run now? › Yes
✔ Workflow dispatched — check progress at https://github.com/owner/repo/actions
```

> [!NOTE]
> **About `COPILOT_GITHUB_TOKEN`**: This is a GitHub Personal Access Token (PAT) that you create, distinct from the automatic `GITHUB_TOKEN` that GitHub Actions provides. It authenticates the Copilot CLI agent running inside the workflow. See [Authentication](/gh-aw/reference/auth/#copilot_github_token) for setup instructions, including which PAT permissions to grant. A paid GitHub Copilot subscription (Individual, Business, or Enterprise) is required to use this engine.
...
3. If you have changed the [frontmatter](/gh-aw/reference/frontmatter/) (the YAML block between `---` delimiters at the top of the file), regenerate the workflow YAML by running:
...
There are hundreds of other ways to use GitHub Agentic Workflows! Browse [Examples](/gh-aw/examples/) and explore the [Agent Factory](/gh-aw/agent-factory-status/) for community-built workflows.
