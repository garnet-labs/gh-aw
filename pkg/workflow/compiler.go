...
func (c *Compiler) generateAndValidateYAML(workflowData *WorkflowData, markdownPath string, lockFile string) (string, error) {
	...
	//
	// Fast-path: skip the expensive YAML parse when no unsafe context expressions appear
	// inside run: blocks.  Unsafe expressions in safe contexts (env:, if:, group:, etc.)
	// are not a template injection concern, so only trigger the parse when they are
	// actually inside a run: block where they could be interpolated into shell commands.
	needsTemplateCheck := hasUnsafeExpressionInRunBlock(yamlContent)
