...
func TestClaudeEngineWithToolsTimeout(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedEnvVar: "GH_AW_TOOL_TIMEOUT: ${{ inputs.tool-timeout }}", // expression passed through
		},
	}
...
func TestCodexEngineWithToolsTimeout(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:            "expression timeout uses default for TOML config",
			toolsTimeout:    "${{ inputs.tool-timeout }}",
			expectedTimeout: "tool_timeout_sec = 60", // expression not evaluable at compile time → default
...
func TestExtractToolsTimeout(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "free-form string - should fail",
			tools: map[string]any{
				"timeout": "not-an-expression",
			},
			expectedTimeout: "",
			shouldError:     true,
		},
