...
func BuildAWFCommand(config AWFCommandConfig) string {
	...
	var command string
	if preamble != "" {
		// Include preamble before AWF command (runs on host before AWF)
...
func BuildAWFArgs(config AWFCommandConfig) []string {
	...
	// (e.g., "${{ vars.OPENAI_BASE_URL }}"), skip it here — BuildAWFCommand will generate a
	...
	// Note: if isEnvVarExpression returns true, workflowData and EngineConfig are guaranteed non-nil.
	copilotTargetIsExpressionEnvVar := isEnvVarExpression(config.WorkflowData, "GITHUB_COPILOT_BASE_URL") &&
		config.WorkflowData.EngineConfig.APITarget == ""
...
func buildExpressionAPITargetArgs(workflowData *WorkflowData) (preamble string, expandableArgs string) {
	...
	for _, s := range specs {
		...
		// Escape any single quotes in the expression so the assignment stays within its
		// single-quoted shell string context. In practice GitHub Actions expressions do not
		// contain single quotes, but this guards against any unusual input.
