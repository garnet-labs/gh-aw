...
`
	...
	if !strings.Contains(agentJobSection, "run: bash /tmp/gh-aw/actions/configure_gh_for_ghe.sh") {
		t.Error("Expected 'run: bash /tmp/gh-aw/actions/configure_gh_for_ghe.sh' in agent job, but it was not found")
	}
